...
```
uv run python scripts/run_experiment.py --name control --prompt-variant v0 --flight-tool-fix 0 --dataset evals/golden_dataset.json --out docs/loop-runs/interview2-final/experiments/control
uv run python scripts/run_experiment.py --name candidate-B-flight-tool-fix --prompt-variant v0 --flight-tool-fix 1 --dataset evals/golden_dataset.json --out docs/loop-runs/interview2-final/experiments/candidate-B-flight-tool-fix
```
...
Generated by claude-opus-4-8 (temperature 0) from the failure clusters and the current source of the relevant surface. This is a DRAFT for human review; the loop does NOT apply it and it is NOT authorized until a human approves.
...
Rationale: The current default path matches flights using an unordered set of cities, which lets reverse-direction routes like "Miami -> New York" satisfy a "New York -> Miami" request. This change replaces the set comparison with an ordered origin-then-destination match and exposes the origin/destination fields so the correct direction is enforced and visible. It mirrors the already-tested behavior behind the FLIGHT_TOOL_FIX flag while touching only the single search_flights tool surface.
